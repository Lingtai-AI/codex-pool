...
class ChainStore:
    ...
    def find_match(self, input_items: list, cfg: dict, eligible_refs: set[str]) -> MatchResult:
        ...
        if not candidates:
            return MatchResult(chain_id=str(uuid.uuid4()), account_ref=None, prefix_hashes=prefix_hashes)
    ...
    def _evict_if_needed(self) -> None:
        # Bounded table: drop the oldest-inserted record on overflow. Restart
        # or eviction transparently loses affinity per contract (not a bug).
